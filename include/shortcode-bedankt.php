...
function fws_create_bedankt($atts) {
	...
	if (preg_match('/^[a-f0-9]{32}$/', $iborderstring, $matches)) {
		...
		if ($order_row = $wpdb->get_row( $sql )) {
			...
			if ($status == 'paid') {
				$content .= '
			<p>'.sprintf(__('Wij hebben de betaling voor uw bestelling ontvangen en zullen het bestelde product of dienst zo snel mogelijk leveren.<br><br>U ontvangt een bevestiging via het opgegeven e-mailadres (%s).', 'fws-ideal-betaalpaginas'), $Emailadres).'</p>';
			} else {
				$content .= '
			<p>'.__('Uw betaling is op dit moment nog niet verwerkt. Wanneer u de betaling heeft geannuleerd, is het helaas niet mogelijk om de betaling opnieuw te beginnen. Heeft u wel betaald, dan ontvangt u alsnog een bevestiging via e-mail.', 'fws-ideal-betaalpaginas').'</p>';
			}
			
			if ($atts['showordersummary'] == 'ja') $content .= '
			<h3>'.__('Uw bestelling:', 'fws-ideal-betaalpaginas').'</h3>
			<p>
